...
| -------- | --------- | ---- | ----------- |
| folder_path |  | string | The suggested slash-delimited subfolder path in which to store this Culture object.  NB: any leading slash should be ignored.  Eg, if folder_path is "/hum/bug" (or "hum/bug") then importing the object into folder "/foo/bar" should result in its folder path being "/foo/bar/hum/bug".  If the importing software does not support folders, then it should ignore this field. |
